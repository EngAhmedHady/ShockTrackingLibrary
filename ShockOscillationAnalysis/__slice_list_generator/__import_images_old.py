...
from ..__generateshocksignal import TimeCalculation

from sklearn.linear_model import LinearRegression
px = 1/plt.rcParams['figure.dpi']

class importSchlierenImagesOld(SOA):
    ...
    def InclinedShockDomainSetup(self, CheckingWidth, CheckingHieght, imgShape, 
       ...
       - It returns a list of Slices location and range, the number of slices, and the inclination applicability.
        ...
        if len(inclinationEst) == 0:  self.LineDraw(self.clone, 'Inc', 3)   
        else:
            P1,P2,m,a = InclinedLine(inclinationEst[1],inclinationEst[2],imgShape = imgShape)
    ...
    def GenerateSlicesArray(self, path, ScalePixels = True, HLP = 0, WorkingRange = [] , FullImWidth = False,
        ...
        if n1 > 1:
            ...
            if len(inclinationEst) > 2 and len(WorkingRange) == 3 and inclinationCheck: 
                ...
                if o%Mode == 0 and n < n1:
                    with open(name):
                        ...
                        if SliceThickness > 0:
                            ...
                            for i in range(SliceThickness): 
                                cropped_image += img[WorkingRange[2]-(Ht+1)+i:WorkingRange[2]-Ht+i,WorkingRange[0]:WorkingRange[1]]
